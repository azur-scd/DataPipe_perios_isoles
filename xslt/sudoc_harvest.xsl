...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <root>
      <xsl:for-each select="ppn">
        ...
        <item>
          ...
          <xsl:call-template name="alephapi">
  ...
  <xsl:template name="alephapi">
    <xsl:param name="ppn"/>
    <xsl:variable name="numaleph" select="document(concat('http://si-scd.unice.fr/applis/webservice/ppn2numaleph.php?ppn=',$ppn))//item/num_aleph"/>
    <num_aleph>
      <xsl:value-of select="$numaleph"/>
    </num_aleph>
  </xsl:template>
</xsl:stylesheet>
<!--
